...
class Radar:
    def __init__(
        ...
        self.rtsp_recorder = recorder or PersistentRtspRecorder(
            self.mediamtx_url,
    ...
    def motion_detected_callback(self, _os=None):
        ...
        if current_time - self.last_image_time >= 3600:
            ...
            if isinstance(latest_lux, (int, float)) and latest_lux > 2:
                logging.info(
                    "Skipping IR for automated picture: luminosity %.2f lux > 2 lux.",
                    float(latest_lux),
                )
            else:
                turn_ir_on()
